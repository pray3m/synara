...
function readProviderUsageSummary(input: {
  ...
  // Captured via an object property: TypeScript resets property narrowing across the
  // render call, while a plain `let` would stay narrowed to null and infer `never`.
  const captured: { summary: ReturnType<typeof useProviderUsageSummary> | null } = {
    summary: null,
  };

  function Probe() {
    captured.summary = useProviderUsageSummary({
      provider: "claudeAgent",
      threads: [],
